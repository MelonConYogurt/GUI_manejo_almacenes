...
class InvoiceGenerator:
    ...
    def download(self, file_path):
        """ Directly send the request and store the file on the specified path """
        json_string = self._to_json()
        response = requests.post(InvoiceGenerator.URL, json=json.loads(json_string), stream=True, headers={'Authorization': 'Bearer sk_g8wybgUY1PTkzyguihD2ZmW7tNOwj2FI'} )
